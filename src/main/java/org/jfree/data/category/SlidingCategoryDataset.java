...
public class SlidingCategoryDataset extends AbstractDataset implements CategoryDataset {
	private static final long serialVersionUID = 3282131296689553558L;
    ...
    public void setFirstCategoryIndex(int first) {
        if(first < 0 || first >= underlying.getColumnCount()) throw new IllegalArgumentException("Invalid index.");
    ...
    public void setMaximumCategoryCount(int max) {
    	if(max < 0) throw new IllegalArgumentException("Requires 'max' >= 0.");
    ...
    private int lastCategoryIndex() {
    	if(this.maximumCategoryCount == 0) return -1;
    ...
    public Comparable getColumnKey(int column) {
    	return underlying.getColumnKey(column + this.firstCategoryIndex);
    }
    ...
    public int getColumnCount() {
        int last = lastCategoryIndex();
        if(last == -1) return 0;
        else return Math.max(last - this.firstCategoryIndex + 1, 0);
    }
